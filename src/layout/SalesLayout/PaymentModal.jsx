...
const PaymentModal = ({ isOpen, onClose, totalAmount, socket }) => {
	...
	const fetchSoldProducts = async () => {
		try {
			...
			setTotalSum(
				parseFloat(datas[sales_id].summa).toFixed(
					...
					<div className="w-1/2 p-5 flex flex-col space-y-4">
						...
						<div className="bg-white rounded-lg border border-gray-200 p-4 shadow-sm">
							...
							<input
								...
								onBlur={() => {
									// Format on blur
									// Ensure cashAmount is a string before replacing
									const valueAsString = String(cashAmount);
									const numericValue =
										parseFloat(
											valueAsString
												.replace(/[^\d,\.]/g, "") // Remove non-numeric/comma/dot chars
												.replace(",", "."), // Replace comma with dot for parseFloat
										) || 0;
									setCashAmount(numericValue); // Store as number
									setIsTyping(false);
								}}
								onFocus={handleFocus}
						...
						<div className="bg-white rounded-lg border border-gray-200 p-4 shadow-sm">
							...
							<input
								...
								onBlur={() => {
									// Format on blur
									// Ensure cardAmount is a string before replacing
									const valueAsString = String(cardAmount);
									const numericValue =
										parseFloat(
											valueAsString
												.replace(/[^\d,\.]/g, "") // Remove non-numeric/comma/dot chars
									...
									setCardAmount(numericValue); // Store as number
